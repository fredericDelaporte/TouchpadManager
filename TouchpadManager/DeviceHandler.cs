...
{
    // Code taken from https://stackoverflow.com/a/1610140/1178314
    ...
    {
        private const string _setupapi = "setupapi.dll";

        private NativeMethods()
        {
        }

        [DllImport(_setupapi, CallingConvention = CallingConvention.Winapi, SetLastError = true)]
        ...
        [DllImport(_setupapi, CallingConvention = CallingConvention.Winapi, CharSet = CharSet.Unicode,
            SetLastError = true)]
...
ref int requiredSize);
    ...
    /// <summary>
    /// Helper for enabling/disabling devices.
    /// </summary>
    public static class DeviceHelper
    {
        ...
        private static int GetIndexOfInstance(SafeDeviceInfoSetHandle handle, DeviceInfoData[] diData,
            string instanceId)
        {
            ...
            for (var index = 0; index <= diData.Length - 1; index++)
            {
                ...
                var result = NativeMethods.SetupDiGetDeviceInstanceId(handle.DangerousGetHandle(), ref diData[index],
                    sb, sb.Capacity, out var requiredSize);
                if (!result && Marshal.GetLastWin32Error() == errorInsufficientBuffer)
                {
